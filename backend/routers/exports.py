...
def download_modelo_111_pdf(
    ...
    pdf.set_font("Helvetica", "", 10)
    base_nom = summary.get('baseNominas', 0.0)
    cuota_nom = summary.get('retencionesNominas', 0.0)
    num_nom = summary.get('numPerceptoresNominas', 0)
    pct_nom = summary.get('pctRetencionNominas', 2)

    base_prof = summary.get('baseProfesionales', 0.0)
    cuota_prof = summary.get('retencionesProfesionales', 0.0)
    num_prof = summary.get('numPerceptoresProfesionales', 0)

    total_ingresar = summary.get('retencionesTotales111', cuota_nom + cuota_prof)
    ...
    pdf.cell(0, 6, "1. DATOS DEL DECLARANTE / RETENEDOR", ln=True)
    pdf.set_font("Helvetica", "", 9)
    pdf.cell(0, 5, f"Razon Social / Nombre: {config.titular if config else 'N/A'}", ln=True)
    pdf.cell(0, 5, f"NIF del Retenedor: {config.nif if config else 'N/A'}", ln=True)
    pdf.cell(0, 5, f"Domicilio Fiscal: {config.direccion if config else 'N/A'}", ln=True)
    pdf.cell(0, 5, f"Periodo de Liquidacion: {periodo}", ln=True)
    pdf.ln(4)

    # SUBSECTION I: RENDIMIENTOS DEL TRABAJO (PLANTILLA)
    pdf.set_font("Helvetica", "B", 10)
    pdf.cell(0, 6, "I. RENDIMIENTOS DEL TRABAJO (Empleados en Plantilla - Clave A)", ln=True)
    pdf.set_fill_color(241, 245, 249)
    pdf.set_font("Helvetica", "B", 8)
    pdf.cell(100, 6, "Concepto", border=1, fill=True)
    pdf.cell(40, 6, "Importe", border=1, fill=True, align="R")
    pdf.ln()

    pdf.set_font("Helvetica", "", 8)
    pdf.cell(100, 6, "Casilla [01] Num. perceptores (registros nomina)", border=1)
    pdf.cell(40, 6, f"{num_nom}", border=1, align="R")
    pdf.ln()
    pdf.cell(100, 6, "Casilla [02] Importe de los rendimientos (salarios netos)", border=1)
    pdf.cell(40, 6, f"{base_nom:.2f} EUR", border=1, align="R")
    pdf.ln()
    pdf.cell(100, 6, "Casilla [03] Retenciones practicadas IRPF (nomina)", border=1)
    pdf.cell(40, 6, f"{cuota_nom:.2f} EUR", border=1, align="R")
    pdf.ln(6)

    # SUBSECTION II: RENDIMIENTOS DE ACTIVIDADES ECONOMICAS (AUTÓNOMOS)
    pdf.set_font("Helvetica", "B", 10)
    pdf.cell(0, 6, "II. RENDIMIENTOS ACTIVIDADES ECONOMICAS (Autonomos Colaboradores - Clave G)", ln=True)
    pdf.set_fill_color(241, 245, 249)
    pdf.set_font("Helvetica", "B", 8)
    pdf.cell(100, 6, "Concepto", border=1, fill=True)
    pdf.cell(40, 6, "Importe", border=1, fill=True, align="R")
    ...
    pdf.set_font("Helvetica", "", 8)
    pdf.cell(100, 6, "Casilla [07] Num. perceptores (facturas autonomos)", border=1)
    pdf.cell(40, 6, f"{num_prof}", border=1, align="R")
    pdf.ln()
    pdf.cell(100, 6, "Casilla [08] Base retenciones facturas profesionales", border=1)
    pdf.cell(40, 6, f"{base_prof:.2f} EUR", border=1, align="R")
    pdf.ln()
    pdf.cell(100, 6, "Casilla [09] Retenciones practicadas en factura (IRPF)", border=1)
    pdf.cell(40, 6, f"{cuota_prof:.2f} EUR", border=1, align="R")
    pdf.ln(8)

    # Resultado
    pdf.set_fill_color(241, 245, 249)
    pdf.set_font("Helvetica", "B", 11)
    pdf.cell(100, 10, "TOTAL A INGRESAR (MODELO 111):", fill=True)
    pdf.cell(40, 10, f"{total_ingresar:.2f} EUR", fill=True, align="R")
    pdf.ln(10)

    pdf.set_font("Helvetica", "I", 7)
    pdf.set_text_color(128, 128, 128)
    pdf.multi_cell(0, 4, "Nota: Los empleados en plantilla figuran como Rendimientos del Trabajo (Clave A). Los autonomos colaboradores figuran como Rendimientos de Actividades Economicas (Clave G). El IVA de facturas de autonomos se deduce en el Modelo 303.")
